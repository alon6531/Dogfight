...
void Map::Load(const char* heightmapPath, Vector3 size, const char* texturePath) {
    ...
    m_airportSize = { 400.0f, 2.0f, 100.0f };
    Vector3 basePos = { -600.0f, 0, -400.0f };
    basePos.y = GetHeightAt(basePos.x, basePos.z) + 25.0f;
...
void Map::Draw() const {
    if (m_isLoaded) {
        ...
        if (m_hasAirport) {
            ...
            DrawCubeWires(m_airportPos, m_airportSize.x, m_airportSize.y + 0.5f, m_airportSize.z, LIGHTGRAY);
        }
    }
}
...
float Map::GetHeightAt(float x, float z) const {
    ...
    if (xRel >= 0.0f && xRel <= 1.0f && zRel >= 0.0f && zRel <= 1.0f) {
        ...
        int pz = (int)(zRel * (m_heightmapImage.height - 1));
        Color col = GetImageColor(m_heightmapImage, px, pz);
        terrainHeight = m_position.y + ((float)col.r / 255.0f) * m_size.y;
    }
